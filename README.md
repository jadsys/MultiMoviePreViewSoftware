...
　MultiMoviePreViewSoftwareは、Windows10で動作するソフトウェアで、ローカルフォルダの複数の動画を、同期を取りながら同時に再生する機能を提供します。同期再生に必要な動画の開始時刻は、動画データベースシステムから合わせて取り出された動画情報ファイル（fffmpeg、MediaInfo等の動画解析・編集ツールにて取得可能）から参照します。統合/個別スライダーを配し、纏めて再生位置を変更することや、個別に再生位置を変更することが可能です。<br>
